...
class CoinHistorical :
        MutableMap<Coin.Companion.price_period, SortedMap<Long, MutableMap<String, Any?>>>, // extends map
        Iterator<MutableMap<String, Any?>> // interfaz iterator
{

    operator fun iterator(): Iterator<MutableMap<String, Any?>> { // we delegate the iterator to the inner class
        return historicalIterator()
    }


    inner class historicalIterator: Iterator<MutableMap<String, Any?>> {
        ...
        private fun findNextIndex(saveResult: Boolean = true) : Long ? {
            ...
            val current_index = this.current_index
            val price_period_values = Coin.Companion.price_period.values()
